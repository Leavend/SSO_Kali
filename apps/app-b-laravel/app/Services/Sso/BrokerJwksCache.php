...
final class BrokerJwksCache
{
    ...
    private function refresh(string $url, ?string $expectedKid): array
    {
        ...
        throw new RuntimeException('The broker JWKS could not be refreshed for the requested key.', 0, $last);
    }
    ...
    private function loadJwks(string $url): array
    {
        ...
        if (! $this->validJwks($jwks)) {
            throw new RuntimeException('The broker JWKS document is invalid.');
        }

        Cache::put($this->cacheKey($url), $jwks, now()->addSeconds($this->cacheTtlSeconds($response)));
    ...
    {
        return 'app-b:broker:jwks:'.sha1($url);
    }

    private function defaultCacheTtlSeconds(): int
    {
        return (int) config('services.sso.jwks.cache_ttl_seconds', 300);
    }

    private function minCacheTtlSeconds(): int
    {
        return max(0, (int) config('services.sso.jwks.min_cache_ttl_seconds', 30));
    }

    private function maxCacheTtlSeconds(): int
    {
        return max($this->minCacheTtlSeconds(), (int) config('services.sso.jwks.max_cache_ttl_seconds', 3600));
    }

    private function maxRefreshAttempts(): int
    {
        return max(1, (int) config('services.sso.jwks.max_refresh_attempts', 2));
    }
}
