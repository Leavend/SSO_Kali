...
const flowKeys = ["organization", "requestId"];
...
function runtime() {
  return `${marker}
;(function(){
if(typeof window==="undefined"||window.__devssoUrlPrivacyVersion==="20260425-dom-safe-v2")return;
window.__devssoUrlPrivacyInjected=true;
window.__devssoUrlPrivacyVersion="20260425-dom-safe-v2";
var KEYS=${JSON.stringify(sensitiveKeys)};
var FLOW_KEYS=${JSON.stringify(flowKeys)};
...
function isLoginTarget(pathname){return /(^|\\/)ui\\/v2\\/login(\\/|$)|\\/(accounts|idp|login|otp|passkey|password|signedin|verify)(\\/|$)/.test(pathname);}
...
function stripEarly(u){var changed=false;KEYS.forEach(function(key){if(FLOW_KEYS.indexOf(key)!==-1||!u.searchParams.has(key))return;u.searchParams.delete(key);changed=true;});return changed;}
function cleanTarget(value){try{var u=new URL(String(value),location.href);if(!isLoginTarget(u.pathname)||!u.search)return value;captureUrl(u);if(!stripEarly(u))return value;return u.pathname+u.search+u.hash;}catch(error){return value;}}
...
function wrap(name){var original=history[name].bind(history);history[name]=function(){var args=Array.prototype.slice.call(arguments);if(args.length>2)args[2]=cleanTarget(args[2]);var result=original.apply(history,args);pulse();return result;};}
