...
function runtime() {
  return `${marker}
;(function(){
if(typeof window==="undefined"||window.__devssoUrlPrivacyVersion==="20260425-auth-flow-v2")return;
window.__devssoUrlPrivacyInjected=true;
window.__devssoUrlPrivacyVersion="20260425-auth-flow-v2";
...
function capture(){try{if(!isLoginPath()||!location.search)return;var u=new URL(location.href),params={};KEYS.forEach(function(key){if(u.searchParams.has(key))params[key]=u.searchParams.get(key);});if(Object.keys(params).length)writeContext({params:params,path:u.pathname,savedAt:Date.now()});}catch(error){}}
...
function pulse(){capture();(isSignedInPath()?[1200,3000,6000,10000]:[250,750,1500,3000,6000,10000]).forEach(function(delay){setTimeout(redact,delay);});}
