...
require_text ".env.dev.example" '^ZITADEL_LOGIN_API_TIMEOUT_MS=6000$' "Development env caps login-to-ZITADEL API latency"
...
require_text "docker-compose.dev.yml" 'ZITADEL_API_TIMEOUT_MS: \$\{ZITADEL_LOGIN_API_TIMEOUT_MS:-6000\}' "Compose passes login-to-ZITADEL API timeout"
...
require_text "scripts/vps-set-login-api-timeout.sh" 'up -d --no-deps zitadel-login-vue' "Login timeout script recreates only the Vue login BFF"
