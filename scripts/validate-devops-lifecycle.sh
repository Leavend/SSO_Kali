...
require_text ".env.dev.example" '^ZITADEL_LOGIN_API_TIMEOUT_MS=15000$' "Development env caps login-to-ZITADEL API latency"
...
require_text "docker-compose.dev.yml" 'ZITADEL_API_TIMEOUT_MS: \$\{ZITADEL_LOGIN_API_TIMEOUT_MS:-15000\}' "Compose passes login-to-ZITADEL API timeout"
...
require_text "services/sso-backend/Dockerfile" 'SSO_BACKEND_OCTANE_WORKERS=1' "SSO backend image defaults Octane to a small-node worker count"
...
require_text "services/zitadel-login-vue/src/server/api-handlers.ts" 'login_api_error' "Vue login logs redacted upstream identity API failures"
...
require_text "scripts/vps-collect-login-flow-logs.sh" 'zitadel-login-vue' "Login-flow log collector covers the Vue login BFF"
...
require_text "scripts/vps-set-login-api-timeout.sh" 'ZITADEL_LOGIN_API_TIMEOUT_MS' "Login timeout script updates only the Vue login upstream timeout"
...
require_text "docker-compose.prod.yml" '^x-runtime-demo-web-budget:' "Production overlay declares constrained demo web budget"
require_text "docker-compose.prod.yml" '^x-runtime-demo-php-budget:' "Production overlay declares constrained demo PHP budget"
