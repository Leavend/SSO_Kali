...
require_text "services/sso-frontend/src/web/styles/auth-shell-floating.css" 'position: sticky' "Vue auth shell toggle uses floating sticky host"
require_text "services/sso-frontend/src/web/styles/auth-shell-floating.css" 'height: 0' "Vue auth shell toggle host does not reserve layout height"
require_text "services/sso-frontend/src/web/styles/auth-shell-floating.css" 'translateY' "Vue auth shell toggle floats away from stacked UI"
require_text "infra/zitadel-login/patch-login-floating-toggle.mjs" 'position: sticky' "Hosted login toggle uses floating sticky host instead of viewport-fixed placement"
