...
require_text "infra/nginx/assets/login/parent-chrome.js" 'cleanNavigationUrl' "Nginx parent chrome fallback strips PII before client-side login navigation"
...
require_text "infra/zitadel-login/patch-login-url-privacy.mjs" 'cleanTarget' "Hosted login URL privacy patch strips PII before SPA history writes"
...
require_text "infra/nginx/dev-sso.timeh.my.id.chained.conf" 'loginName' "Nginx detects hosted login PII query strings before rendering HTML"
