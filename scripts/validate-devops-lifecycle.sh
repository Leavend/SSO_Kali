...
require_text ".github/workflows/ci.yml" 'sso-admin-frontend' "CI builds Vue admin canary image"
...
require_text "docker-compose.dev.yml" 'sso-admin-frontend:' "Compose defines Vue canary service"
...
require_text "services/sso-backend/app/Services/Oidc/ClientIntegrationContractBuilder.php" 'Client ID sudah terdaftar di SSO' "SSO backend rejects duplicate client registration"
...
require_text "services/sso-backend/tests/Feature/Admin/AdminClientIntegrationContractTest.php" 'without mutating session clients' "SSO backend tests readonly client integration contract validation"
...
require_text "docker-compose.dev.yml" 'image: sso-dev-sso-admin-frontend:\$\{APP_IMAGE_TAG:-local\}' "Admin frontend image is tag-addressable"
require_text "docker-compose.dev.yml" 'Host\(`\$\{SSO_ADMIN_DOMAIN:-admin-sso.timeh.my.id\}`\)' "Admin frontend route uses dedicated host"
require_text "scripts/vps-apply-sso-admin-frontend-route.sh" 'assert_no_edge_auth_middleware' "Admin frontend VPS route audits edge auth middleware"
require_text "scripts/vps-apply-sso-admin-frontend-route.sh" 'auth_request\|oauth2-proxy\|forwardAuth\|ForwardAuth\|callbacks/upstream\|oauth/v2/authorize\|zitadel' "Admin frontend VPS route blocks external auth middleware tokens"
require_text ".github/workflows/deploy-main.yml" 'expected /auth/login not to redirect through edge auth middleware' "Admin frontend deploy smoke rejects edge auth redirect"
require_text ".github/workflows/deploy-main.yml" 'client_id=sso-admin-panel' "Admin frontend deploy smoke verifies BFF OIDC client_id"
...
require_text "docker-compose.dev.yml" 'traefik.http.routers.sso-admin-frontend.priority=175' "Admin frontend priority stays below backend"
require_text ".env.dev.example" '^SSO_ADMIN_DOMAIN=admin-sso.timeh.my.id$' "Development env example exposes admin frontend domain"
require_text ".env.dev.example" '^SSO_ADMIN_FRONTEND_BASE_PATH=/$' "Development env example builds admin frontend for root host"
...
require_text "scripts/vps-deploy.sh" 'sso-admin-frontend' "Deploy includes Vue admin canary"
...
require_text "scripts/vps-direct-build-deploy.sh" 'Admin Frontend' "Direct VPS deploy smokes admin frontend domain"
...
require_text ".github/workflows/devops-lifecycle.yml" 'services/sso-admin-frontend/\*\*' "DevOps workflow watches Vue Admin lifecycle changes"
