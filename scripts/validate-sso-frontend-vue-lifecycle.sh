...
require_text "services/sso-frontend/src/web/views/LoginView.vue" "AUTH_SHELL.copy.loginTitle" "Vue login headline uses parent copy token"
require_text "services/sso-frontend/src/web/views/LoginView.vue" "AUTH_SHELL.copy.forgotPassword" "Vue login password reset action uses parent copy token"
require_text "services/sso-frontend/src/web/views/LoginView.vue" "AUTH_SHELL.copy.registerAction" "Vue login registration action uses parent copy token"
...
require_text "services/sso-frontend/src/web/styles/auth-shell-floating.css" ".app-shell--auth" "Vue auth surface is viewport locked"
require_text "services/sso-frontend/src/web/styles/auth-shell-floating.css" "max-height: 100dvh" "Vue auth surface cannot exceed one viewport"
require_text "services/sso-frontend/src/web/styles/auth-shell-floating.css" "env\\(safe-area-inset-bottom" "Vue auth floating actions respect bottom safe area"
reject_text "services/sso-frontend/src/web/styles/auth-shell-floating.css" "--auth-frame-width|100vw - var\\(--auth-frame-width\\)" "Vue auth floating actions are not anchored to the login card"
reject_text "services/sso-frontend/src/web/components/FloatingActions.vue" "ResizeObserver|requestAnimationFrame|--floating-actions-lift" "Vue auth floating actions avoid runtime layout polling"
reject_text "services/sso-frontend/src/web/styles/auth-shell-floating.css" "--floating-actions-lift" "Vue auth floating actions do not keep stale dynamic lift hooks"
