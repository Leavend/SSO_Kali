...
require_text "docker-compose.dev.yml" 'PathPrefix\(`/ui/v2/login-vue`\)' "Vue login legacy route redirects without downtime"
require_text ".env.dev.example" '^ZITADEL_LOGIN_ACTIVE_BASE_PATH=/ui/v2/login$' "Env defaults to hosted login for rollback safety"
require_text ".env.dev.example" '^ZITADEL_LOGIN_VUE_BASE_PATH=/ui/v2/auth$' "Env exposes Vue login canary path"
require_text ".env.dev.example" '^ZITADEL_LOGIN_REQUIRE_TOTP_AFTER_PASSWORD=true$' "Env keeps Vue login password-to-TOTP flow enabled"
...
require_text "services/zitadel-login-vue/src/server/api-handlers.ts" "nextStep: 'otp'" "Password success routes to authenticator code input"
...
require_text "scripts/vps-deploy.sh" 'ZITADEL_LOGIN_VUE_LEGACY_BASE_PATH="/ui/v2/login-vue"' "Registry deploy migrates legacy Vue login path safely"
...
require_text "scripts/vps-login-ui-cutover.sh" 'ZITADEL_LOGIN_VUE_BASE_PATH' "Login UI cutover keeps Vue canary path aligned"
