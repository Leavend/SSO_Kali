...
| BE-FR038-001 | `app/Support/Security/TokenLifetimePolicy.php`, `app/Actions/Security/ValidateTokenLifetimePolicyAction.php`, `app/Console/Commands/CheckTokenLifetimePolicyCommand.php` (`sso:check-token-lifetime-policy`) | `tests/Feature/Security/TokenLifetimePolicyContractTest.php` | `25e755d` |
...
| BE-FR040-001 | `database/migrations/2026_05_16_000001_create_oidc_rp_sessions_table.php`, `app/Services/Oidc/BackChannelSessionRegistry.php` (DB source of truth + cache acceleration) | `tests/Feature/Oidc/RpSessionRegistryPersistenceContractTest.php` | `25e755d` |
...
| BE-FR043-001 | `database/migrations/2026_05_16_000002_add_frontchannel_logout_to_oidc_client_registrations.php`, `app/Support/Oidc/DownstreamClient.php` (`frontchannelLogoutUri`/`hasValidFrontchannelLogoutUri`/`supportsLogoutNotification`), `app/Services/Oidc/DownstreamClientRegistry.php` (config + dynamic FCL wiring), `app/Services/Oidc/OidcCatalog.php` (advertises `frontchannel_logout_supported`), `app/Actions/Oidc/RegisterClientSession.php` (accepts FCL-only RPs), `app/Services/Oidc/BackChannelLogoutDispatcher.php` (returns `frontchannel_pending`), `app/Actions/Oidc/RenderFrontChannelLogoutFallback.php` + `app/Http/Controllers/Oidc/FrontChannelLogoutFallbackController.php` (OIDC FCL §3 iframe page) | `tests/Feature/Oidc/FrontChannelLogoutFallbackContractTest.php` | `b0cca06`, `c1ebabe` |
| BE-FR049-001 | `database/migrations/2026_05_17_000003_create_data_subject_requests_table.php`, `app/Models/DataSubjectRequest.php`, `app/Services/DataSubject/DataSubjectRequestService.php` (submit → review → fulfill state machine + SLA timestamps + audit), `app/Services/DataSubject/DataSubjectExportBuilder.php` (PII-redacted export bundle), `app/Http/Controllers/DataSubject/DataSubjectRequestController.php` (self-service via `ProfilePrincipalResolver`), `app/Http/Controllers/Admin/DataSubjectRequestAdminController.php` (review/fulfill under `admin.dsr.*` permissions) | `tests/Feature/Admin/AdminGovernanceContractTest.php` | `<pending>` |
| BE-FR050-001 | `app/Services/Admin/AdminDashboardSummaryService.php` (cached/bounded counters), `app/Http/Controllers/Admin/DashboardSummaryController.php`, route `GET /admin/api/dashboard/summary` gated by `admin.dashboard.view` | `tests/Feature/Admin/AdminGovernanceContractTest.php` | `<pending>` |
| BE-FR051-001 | `database/migrations/2026_05_17_000001_add_lock_fields_to_users.php`, `app/Models/User.php` (lock fields/casts), `app/Actions/Admin/LockManagedUserAction.php` + `app/Actions/Admin/UnlockManagedUserAction.php`, `app/Http/Controllers/Admin/UserLifecycleLockController.php`, `app/Services/Admin/AdminUserPresenter.php` (locks visible to admins) | `tests/Feature/Admin/AdminGovernanceContractTest.php` | `<pending>` |
| BE-FR052-001 | `database/migrations/2026_05_17_000002_add_signing_key_id_to_admin_audit_events.php`, `app/Support/Audit/AdminAuditSigningKeyRegistry.php` (versioned HMAC key id), `app/Services/Admin/AdminAuditEventStore.php` + `app/Services/Admin/AdminAuditIntegrityVerifier.php` (key-id aware hash), `app/Actions/Admin/ExportAdminAuditEventsAction.php` (CSV/JSONL field-whitelist export with audit row), `app/Console/Commands/PruneAdminAuditEventsCommand.php` (>= 365 day retention floor + dry-run + capped batch) | `tests/Feature/Admin/AdminGovernanceContractTest.php` | `<pending>` |
| BE-FR053-001 | `app/Support/Rbac/AdminPermission.php` (`audit.export`/`dsr.read`/`dsr.review`/`dashboard.view`/`users.lock` + role catalogs), `database/seeders/RbacSeeder.php` (seeds `auditor`, `support`, `client-manager`, `security-officer`) | `tests/Feature/Admin/AdminGovernanceContractTest.php` | `<pending>` |
| BE-FR059-001 | `database/migrations/2026_05_19_000001_add_circuit_breaker_to_external_identity_providers.php` (consecutive_failures/successes + breaker_tripped_at/reason), `app/Services/ExternalIdp/ExternalIdpHealthProbeService.php` (counter accounting + breaker tripping), `app/Services/ExternalIdp/ExternalIdpFailoverPolicy.php` + `app/Services/ExternalIdp/ExternalIdpAuthenticationRedirectService.php` (breaker-aware eligibility), `app/Console/Commands/ProbeExternalIdpHealthCommand.php` + `routes/console.php` (`sso:external-idp:probe-health` schedule every 5 min), `config/sso.php` (`sso.external_idp.failure_threshold`) | `tests/Feature/ExternalIdp/ExternalIdpCircuitBreakerContractTest.php` | `<pending>` |
| BE-FR060-001 | `app/Support/Oidc/OidcErrorCatalog.php` (RFC 6749/6750 + OIDC Core + SSO codes w/ status, copy_key, retryability, support_action, source) | `tests/Feature/Oidc/OidcErrorCatalogContractTest.php` | `<pending>` |
...
| BE-FR058-001 | `app/Services/ExternalIdp/ExternalIdpClaimsMapperPreviewService.php` (mapping preview projector + missing-email policy resolver), `app/Http/Controllers/Admin/ExternalIdentityProviderMappingPreviewController.php` (admin preview endpoint), `routes/admin.php` registers `POST /admin/api/external-idps/{providerKey}/mapping-preview` under `admin.external-idps.write` | `tests/Feature/Admin/SecurityPolicyAndFederationContractTest.php` | `<pending>` |
| BE-FR044-001 | `app/Actions/Oidc/PerformSingleSignOut.php` (`sid|sub|request_id` idempotency cache), `app/Services/Oidc/BackChannelLogoutDispatcher.php` (parent request-id propagation), `app/Jobs/DispatchBackChannelLogoutJob.php` (`requestId` payload + `backchannel_logout_dead_lettered` terminal audit on exhausted retries) | `tests/Feature/Oidc/CentralizedLogoutTest.php` | `<pending>` |
| BE-FR045-001 | `app/Services/Profile/ProfilePortalPresenter.php` (scope-minimized portal payload), `app/Support/Oidc/ClaimsView.php` + `app/Actions/Oidc/BuildUserInfo.php` (scope-minimized UserInfo), `app/Actions/Audit/RecordAuthenticationAuditEventAction.php` and `app/Actions/Audit/RecordLogoutAuditEventAction.php` (redaction/hash controls) | `tests/Feature/Profile/ProfilePortalBackendContractTest.php`, `tests/Feature/Oidc/UserInfoPassportEmptyScopeContractTest.php`, `tests/Feature/Profile/ConnectedAppsSelfServiceRevocationContractTest.php` | `<pending>` |
| BE-FR046-001 | `app/Actions/Admin/SyncManagedUserProfileAction.php` (email-change clears `email_verified_at`), `app/Actions/Profile/UpdateProfilePortalAction.php` (self-service update remains name-only + audited), `app/Services/Admin/AdminUserPresenter.php` (verification state visible to admins) | `tests/Feature/Admin/UserManagementBackendTest.php`, `tests/Feature/Profile/ProfilePortalBackendContractTest.php` | `<pending>` |
| BE-FR047-001 | `app/Http/Controllers/Resource/ChangePasswordController.php` (current-password verification, strong-password rule, Argon hash, refresh/access/session revocation, audit event) | `tests/Feature/Profile/ChangePasswordContractTest.php`, `tests/Feature/Admin/UserManagementBackendTest.php` | `<pending>` |
| BE-FR048-001 | `app/Services/Profile/ConnectedAppsService.php` (refresh-token + consent + RP-session registry aggregation), `app/Services/Profile/UserSessionsService.php` (RP session registry visibility + device metadata shape), `app/Actions/Profile/RevokeConnectedAppAction.php` + `app/Services/Oidc/AccessTokenRevocationStore.php` (subject+client-scoped access token revocation) | `tests/Feature/Profile/ConnectedAppsSelfServiceRevocationContractTest.php`, `tests/Feature/Profile/UserSessionsSelfServiceContractTest.php`, `tests/Feature/Profile/ProfilePortalDataQualityContractTest.php` | `<pending>` |
| FE-FR045-001 | `services/sso-frontend/src/pages/portal/ProfilePage.vue` (approved-field-only profile summary, optional-field fallback, shared safe error presenter), `services/sso-frontend/src/lib/api/safe-error-presenter.ts` | `services/sso-frontend/src/pages/portal/profile-privacy.contract.test.ts` | `<pending>` |
| BE-FR047-001 / FE-FR047-001 | `app/Actions/Auth/RequestPasswordResetAction.php`, `app/Actions/Auth/ConfirmPasswordResetAction.php`, `app/Actions/Auth/RevokePasswordResetSessionsAction.php`, `app/Http/Controllers/Auth/PasswordReset{Request,Confirm}Controller.php`, `routes/auth.php` (`POST /api/auth/password-reset`, `POST /api/auth/password-reset/confirm`), `services/sso-frontend/src/composables/usePasswordLifecycle.ts`, `services/sso-frontend/src/pages/auth/{ForgotPasswordPage,ResetPasswordPage}.vue`, `services/sso-frontend/src/pages/portal/SecurityPage.vue` | `tests/Feature/Auth/PasswordResetLifecycleContractTest.php`, `tests/Feature/Profile/ChangePasswordContractTest.php`, `services/sso-frontend/src/composables/use-password-lifecycle.contract.test.ts` | `<pending>` |
| FE-FR049-001 | `services/sso-frontend/src/pages/portal/PrivacyPage.vue`, `services/sso-frontend/src/composables/useDataSubjectRequests.ts`, `services/sso-frontend/src/services/profile.api.ts` (`getDataSubjectRequests`, `createDataSubjectRequest`), `services/sso-frontend/src/router/index.ts` (`portal.privacy`), `services/sso-frontend/src/components/organisms/PortalHeader.vue` (Privasi nav) | `services/sso-frontend/src/pages/portal/privacy-page.contract.test.ts` | `<pending>` |
| FE-FR050-001 | `services/sso-frontend/src/components/layouts/AdminLayout.vue` (admin shell + capability-filtered nav), `services/sso-frontend/src/pages/admin/AdminDashboardPage.vue` (summary cards + safe state), `services/sso-frontend/src/composables/useAdminDashboard.ts`, `services/sso-frontend/src/services/admin-dashboard.api.ts`, `services/sso-frontend/src/server/admin-{api,handlers}.ts` (BFF proxy to `/admin/api/dashboard/summary`) | `services/sso-frontend/src/pages/admin/admin-dashboard.contract.test.ts`, `services/sso-frontend/src/__tests__/router-public-portal.test.ts` | `<pending>` |
| FE-FR051-001 | `services/sso-frontend/src/pages/admin/UserManagementPage.vue`, `services/sso-frontend/src/composables/useAdminUsers.ts`, `services/sso-frontend/src/services/admin-users.api.ts` (create/profile/deactivate/reactivate/lock/unlock/password-reset), `services/sso-frontend/src/stores/admin-console.store.ts` (capability gates) | `services/sso-frontend/src/pages/admin/user-management.contract.test.ts` | `<pending>` |
| FE-FR052-001 | `services/sso-frontend/src/pages/admin/AuditTrailPage.vue`, `services/sso-frontend/src/composables/useAdminAuditTrail.ts`, `services/sso-frontend/src/services/admin-audit.api.ts` (filters/integrity/export blob), `services/sso-frontend/src/server/admin-{api,handlers}.ts` (BFF proxy to audit events/integrity/export) | `services/sso-frontend/src/pages/admin/audit-trail.contract.test.ts` | `<pending>` |
