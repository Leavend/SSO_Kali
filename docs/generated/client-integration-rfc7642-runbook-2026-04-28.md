...
- Repository hygiene: dependency snapshot, copy folder seperti `node_modules 2`, local archive `.zip`/`.tar.gz`, dan build artifact lokal tidak boleh menjadi source of truth. CI menjalankan guard untuk memastikan artifact tersebut tidak tracked dan Docker context tidak membawanya ke image build.
