...
- [ ] **Invariant host widget:** panggil `/widget/*` ke host yang menyajikan login interaktif dan menyimpan `__Host-sso_session` (= `sso.timeh.my.id` / `SSO_WIDGET_PUBLIC_BASE_URL`). **Jangan** memanggil host API/issuer (`api-sso` / `VITE_SSO_BASE_URL` / `VITE_OIDC_ISSUER`) untuk widget — cookie `__Host-` bersifat host-only, jadi panggilan widget lintas-host tidak membawa sesi dan balas `401`.
