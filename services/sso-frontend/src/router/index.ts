...
router.beforeEach(async (to) => {
  ...
  if (to.meta.requiresGuest) {
    ...
    // Optimistic: if we already know the user, redirect. Otherwise let the
    // guest page render first and hydrate the session in the background so
    // LCP isn't blocked by the server session check. If the background
    // hydration later confirms a session, we redirect post-render.
    if (session.isAuthenticated) {
      return { name: 'portal.home' }
    }
    void session.ensureSession().then((authenticated) => {
      if (authenticated) {
        void router.replace({ name: 'portal.home' })
      }
    })
  }
