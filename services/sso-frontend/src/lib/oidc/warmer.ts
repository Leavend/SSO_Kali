...
function reportCanonicalizationViolation(error: CanonicalizationError): void {
  console.error(`[OIDC] Discovery canonicalization violation (${error.code}): ${error.message}`)
}
...
async function prefetchQuietly(): Promise<void> {
  try {
    ...
    const [discovery] = await Promise.allSettled([fetchDiscovery(discoveryUrl), fetchJwks(jwksUrl)])
