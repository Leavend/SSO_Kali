...
export function isForbiddenApiError(error: unknown): boolean {
  return isAdminApiError(error) && (error.status === 403 || error.code === "forbidden");
}
...
function fallbackMessage(status: number): string {
  ...
  if (status === 404) {
    return "The requested admin resource could not be found.";
  }
  ...
  if (status >= 500) {
    return "The admin API is temporarily unavailable. Please try again.";
  }
