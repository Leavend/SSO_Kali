...
describe("broker boundary scan", () => {
  ...
  it("allows admin panel config that targets the broker", async () => {
    const dir = await fixtureDir("allowed");

    await writeSource(dir, "src/lib/config.ts", "export const base = 'https://dev-sso.timeh.my.id';\n");
  ...
  it("rejects direct zitadel env bindings and upstream endpoints", async () => {
    const dir = await fixtureDir("blocked");

    await writeSource(
      dir,
      ".env.example",
      "NEXT_PUBLIC_ZITADEL_ISSUER=https://id.dev-sso.timeh.my.id\nNEXT_PUBLIC_TOKEN_URL=https://id.dev-sso.timeh.my.id/oauth/v2/token\n",
    );

    const result = await runScan(dir);

    expect(result.exitCode).toBe(1);
    expect(result.stderr).toContain("zitadel-env-binding");
  });
});

async function fixtureDir(prefix: string): Promise<string> {
  const dir = await mkdtemp(path.join(tmpdir(), `admin-broker-boundary-${prefix}-`));
