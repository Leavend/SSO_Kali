...
it('creates an external idp registry record with secure production defaults', function (): void {
    $provider = app(CreateExternalIdentityProviderAction::class)->execute(externalIdpExternalIdpPayload());
...
it('writes redacted tamper-evident audit evidence when registering an external idp', function (): void {
    app(CreateExternalIdentityProviderAction::class)->execute(externalIdpExternalIdpPayload());
    ...
    expect($event->action)->toBe('external_idp.create')
        ->and($event->admin_subject_id)->toBe('admin-externalIdp')
 ...
 */
function externalIdpExternalIdpPayload(): array
{
    return [
