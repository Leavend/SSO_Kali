...
it('keeps main production compose backend-only with queue worker and no admin UI', function (): void {
...
it('deploys only backend production services and removes admin-vue orphans', function (): void {
    ...
    expect($deploy)->toBeString()
        ...
        ->and($deploy)->toContain('compose up -d --remove-orphans sso-backend sso-backend-worker')
