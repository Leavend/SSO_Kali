...
it('keeps oidcBackend production smoke secret-free and protocol complete', function (): void {
    ...
    expect($script)->toBeFile()
        ...
        ->and($content)->toContain('require_native_login_redirect')
        ->and($content)->toContain('authorize native login')
        ->and($content)->toContain('sso[.]timeh[.]my[.]id/login')
        ->and($content)->toContain('/oauth/v2/authorize')
        ->and($content)->toContain('/callbacks/upstream')
        ->and($content)->toContain('client_id=&')
        ->and($content)->toContain('return_to')
