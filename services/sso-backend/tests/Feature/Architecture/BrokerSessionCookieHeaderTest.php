...
it('sets a host-prefixed secure session cookie on authorize redirects', function (): void {
    /** @var TestCase $this */
    Http::fake([
    ...
    $cookie = sessionCookie($response->headers->getCookies());

    expect($cookie)->not->toBeNull();
    expect($cookie?->getName())->toBe('__Host-broker_session');
 ...
 */
function sessionCookie(array $cookies): ?Cookie
{
