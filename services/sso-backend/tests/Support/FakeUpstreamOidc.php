<?php

declare(strict_types=1);

namespace Tests\Support;

use Firebase\JWT\JWT;

final class FakeUpstreamOidc
{
    private const PRIVATE_KEY = <<<'PEM'
-----BEGIN PRIVATE KEY-----

UdoH2sa5F0/pP0uk3nCEAHLn
-----END PRIVATE KEY-----
PEM;

    /**
     * @return array<string, mixed>
     */
    public static function jwks(string $kid = 'upstream-kid'): array
    {
        return [
            'keys' => [[
                'kty' => 'RSA',
                'use' => 'sig',
                'kid' => $kid,
                'alg' => 'RS256',
                'n' => 'udSZGZuTa9j-LNi16C7WieEflZxjSRTfeI15-Wh346vMMeLaM1fkdBIzVwWz4aZOnpZAr8_ZM_MWDDt9I0BmU-gmaJYFO2SryqnbvhrrZ3dML5tvZjdXwLvA3NFDxYG04YWNVFcoC4J1Uflj-YOK5AgxKK2Y5PsBjAdMW_1RcS7jPnSxGtjYd9uklt4YM1csn4KDTXvF7-3NbdeWOjLtU2pv26IgEaNFefd5V9YekIOtWuW_cvKFEI4ocn4wdem6-3t_81WGrLK_O5zMiYDdnqe_FQzca3bJrcLTmkSf3rDK3gsTQHnvfeIRfZOUBrFx3uR_77pc3ltyQEA-jR4rOw',
                'e' => 'AQAB',
            ]],
        ];
    }

    public static function idToken(string $nonce, array $overrides = [], string $kid = 'upstream-kid'): string
    {
        return JWT::encode(
            array_replace(self::idTokenClaims($nonce), $overrides),
            self::PRIVATE_KEY,
            'RS256',
            $kid,
        );
    }

    public static function expiredIdToken(string $nonce): string
    {
        return self::idToken($nonce, [
            'iat' => time() - 600,
            'exp' => time() - 300,
        ]);
    }

    public static function tamperedIdToken(string $nonce): string
    {
        $parts = explode('.', self::idToken($nonce));
        $parts[2] = 'invalid-signature';

        return implode('.', $parts);
    }

    public static function algorithmToken(string $nonce, string $algorithm): string
    {
        return self::manualToken(array_replace(
            self::idTokenClaims($nonce),
            ['exp' => time() + 300],
        ), $algorithm);
    }

    /**
     * @return array<string, mixed>
     */
    public static function idTokenClaims(string $nonce): array
    {
        return [
            'iss' => 'https://zitadel.example',
            'aud' => 'broker-client',
            'sub' => '47c0cf0b-8af7-4af6-b2cb-1fd8cf4f8d8c',
            'nonce' => $nonce,
            'auth_time' => time() - 10,
            'amr' => ['pwd'],
            'acr' => 'urn:zitadel:loa:1',
            'iat' => time() - 5,
            'exp' => time() + 300,
        ];
    }

    /**
     * @param  array<string, mixed>  $claims
     */
    private static function manualToken(array $claims, string $algorithm): string
    {
        $header = ['typ' => 'JWT', 'alg' => $algorithm];

        return sprintf(
            '%s.%s.%s',
            self::encodeSegment($header),
            self::encodeSegment($claims),
            $algorithm === 'none' ? '' : 'signature',
        );
    }

    /**
     * @param  array<string, mixed>  $value
     */
    private static function encodeSegment(array $value): string
    {
        return rtrim(strtr(base64_encode(json_encode($value, JSON_THROW_ON_ERROR)), '+/', '-_'), '=');
    }
}
