...
describe('ssoErrorTemplatesApi', () => {
  ...
  it('scopes the list to a locale via ?locale when one is given', async () => {
    const get = vi.spyOn(apiClient, 'get').mockResolvedValue({ templates: [] } as never)
    await ssoErrorTemplatesApi.list('en')
    expect(get).toHaveBeenCalledWith('/api/admin/sso-error-templates?locale=en')
  })
