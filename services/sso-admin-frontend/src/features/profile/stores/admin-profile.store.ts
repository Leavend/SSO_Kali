import { ref } from 'vue'
import { defineStore } from 'pinia'
import { ApiError, getLastRequestId } from '@/lib/api/api-client'
import {
  isAdminProxyTransportFailure,
  formatTransportErrorMessage,
} from '@/lib/display-identifiers'
import { profileApi } from '../services/profile.api'
import type { AdminPrincipal } from '../types'

export type AdminProfileStatus =
  | 'idle'
  | 'loading'
  | 'success'
  | 'unauthenticated'
  | 'forbidden'
  | 'error'

export const useAdminProfileStore = defineStore('admin-profile', () => {
  const status = ref<AdminProfileStatus>('idle')
  const principal = ref<AdminPrincipal | null>(null)
  const errorMessage = ref<string | null>(null)
  const requestId = ref<string | null>(null)

  async function load(): Promise<void> {
    status.value = 'loading'
    errorMessage.value = null

    try {
      const response = await profileApi.getProfile()
      principal.value = response.principal
      requestId.value = getLastRequestId()
      status.value = 'success'
    } catch (error) {
      principal.value = null
      handleLoadError(error)
    }
  }

  function handleLoadError(error: unknown): void {
    if (error instanceof ApiError) {
      requestId.value = error.requestId ?? getLastRequestId()

      if (error.status === 401) {
        status.value = 'unauthenticated'
        errorMessage.value = 'Sesi admin berakhir. Login ulang untuk melanjutkan.'
        return
      }

      if (error.status === 403) {
        status.value = 'forbidden'
        errorMessage.value = 'Kamu tidak memiliki izin untuk melihat profil admin.'
        return
      }
    } else {
      requestId.value = getLastRequestId()
    }

    status.value = 'error'
    if (isAdminProxyTransportFailure(error)) {
      errorMessage.value =
        formatTransportErrorMessage(requestId.value) ?? 'Profil admin belum bisa dimuat.'
    } else {
      errorMessage.value = requestId.value
        ? `Profil admin belum bisa dimuat. Gunakan request ID ${requestId.value} untuk investigasi.`
        : 'Profil admin belum bisa dimuat. Coba lagi beberapa saat lagi.'
    }
  }

  return {
    status,
    principal,
    errorMessage,
    requestId,
    load,
  }
})
